...
from .models import Base, Trip, Vehicle, Place, OdometerSnapshot
...
async def update_trip(trip_id: int = Path(...), payload: TripIn = None, db: Session = Depends(get_db)):
    ...
    trip.distance_km = payload.distance_km or trip.distance_km
...
@app.get("/summary/year", response_model=YearSummary)
async def year_summary(vehicle: str = Query(...), year: int = Query(...), db: Session = Depends(get_db)):
    veh = db.query(Vehicle).filter(Vehicle.reg_no==vehicle).first()
    if not veh:
        raise HTTPException(404, "Vehicle not found")
    start = datetime(year,1,1); end = datetime(year+1,1,1)
    q = db.query(Trip).filter(Trip.vehicle_id==veh.id, Trip.started_at>=start, Trip.started_at<end)
    trips = q.count()
    biz_km = sum([t.distance_km or 0 for t in q.filter(Trip.business==True)])
    priv_km = sum([t.distance_km or 0 for t in q.filter(Trip.business==False)])
    return YearSummary(vehicle_reg=vehicle, year=year, business_km=biz_km, private_km=priv_km, trips=trips)
